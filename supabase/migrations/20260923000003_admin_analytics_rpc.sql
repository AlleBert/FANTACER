-- Metriche admin aggregate nel DB (niente scan lato app, niente cap 1000).
-- Semantica: "scheda effettiva" = vote_sessions in batch con nessuna delle 3
-- aziende bloccata. Override punteggio = base + delta (come get_company_ranking).
-- Idempotente, RLS/grant service_role.

-- 1) Summary per KPI card + grafico.
...
-- 2) Statistiche per-azienda (pagina Aziende): punti effettivi + trend.
