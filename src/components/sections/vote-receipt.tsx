...
export function VoteReceipt({ companies }: VoteReceiptProps) {
  ...
  return (
    <div className="w-full rounded-(--rounded-lg) border-[3px] border-ink bg-white p-(--space-lg) text-left shadow-[4px_4px_0_#000]">
      ...
      <ul className="flex flex-col gap-(--space-sm)">
        {companies.map((item, index) => (
          <li key={index} className="flex items-center justify-between gap-(--space-sm)">
            <span className="truncate text-(length:--fs-body) font-bold text-ink">{item.company.name}</span>
